...
"""Test on an imdb (image database), for test set (mAP)."""
...
def _get_image_blob(im):
    ...
    """
    im_orig = im.astype(np.float32, copy=False)
...
def test_net(net, imdb, run_name):
    ...
    print 'Applying NMS to all detections', cfg.TEST.NMS
    nms_dets = apply_nms(all_boxes, cfg.TEST.NMS)

    nms_file = os.path.join(output_dir, 'nms_detections.pkl')
