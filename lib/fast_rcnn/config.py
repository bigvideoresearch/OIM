...
This file specifies default config options for OIM.
"""
...
# 1/8 is correct for VGG_CNN_M_1024 and VGG16
__C.DEDUP_BOXES = 1./8.
...
def get_output_dir(imdb, net, run_name):
    """Return the directory where experimental artifacts are placed.

    A canonical path is built using the name from an imdb and a network
    (if not None).
    """
    # explicit indicate of run name as experiment dir
    path = osp.abspath(osp.join(__C.ROOT_DIR, 'output', run_name, imdb.name))
    if net is None:
        return path
    else:
