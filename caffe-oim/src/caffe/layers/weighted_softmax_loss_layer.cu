...
__global__ void WeightedSoftmaxLossForwardGPU(const int nthreads,
          ...
          const Dtype* prob_ratio, const Dtype* alpha, const Dtype* fg_num, const Dtype* beta,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    if (has_ignore_label_ && label_value == ignore_label_) {
      ...
      loss[index] = -weights[n] * log(max(prob_data[n * dim + label_value * spatial_dim + s], Dtype(FLT_MIN)))
        -weights[n] * alpha[0] * log(max(prob_ratio[n], Dtype(FLT_MIN)));
      if (weights[n] > 1e-12) {
        counts[index] = 1;
      }
    }
...
void WeightedSoftmaxWithLossLayer<Dtype>::Forward_gpu(
  ...
  const Dtype* prob_ratio = bottom[3]->gpu_data();
  const Dtype* alpha = bottom[4]->gpu_data();
  const Dtype* fg_num = bottom[5]->gpu_data();
  const Dtype* beta = bottom[6]->gpu_data();
...
__global__ void WeightedSoftmaxLossBackwardGPU(const int nthreads, const Dtype* top,
          const Dtype* label, const Dtype* weights,
          const Dtype* prob_ratio, const Dtype* alpha, const Dtype* fg_num, const Dtype* beta,
          Dtype* bottom_diff, const int num,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    if (has_ignore_label_ && label_value == ignore_label_) {
      ...
      for (int c = 0; c < channels; ++c) {
        if (1.0 - prob_ratio[n] < 1e-3) {
          // suppress: maximum, background
          if (fg_num[n] > 1e-3) {
            bottom_diff[n * dim + c * spatial_dim + s] *= weights[n] * (- beta[0]);
          } else {
            bottom_diff[n * dim + c * spatial_dim + s] *= weights[n];
          }
        } else {
          // depress: only for large prob
          if (prob_ratio[n] > 0.3 - 1e-3) {
            bottom_diff[n * dim + c * spatial_dim + s] *= weights[n] * (1.0 + alpha[0]);
          } else {
            bottom_diff[n * dim + c * spatial_dim + s] *= weights[n];
          }
        }
...
void WeightedSoftmaxWithLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    WeightedSoftmaxLossBackwardGPU<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
        CAFFE_CUDA_NUM_THREADS>>>(nthreads, top_data, label, cls_loss_weights,
        prob_ratio, alpha, fg_num, beta, bottom_diff,
